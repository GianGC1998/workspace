...
const Page: FC = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () =>
      api.providerControllerFindAll({ page, search: debouncedSearch }),
    select: (data) => data.data,
  });

  return (
    <SectionContainer title={t('navigation.providers')} error={error?.message}>
      <SearchAndCreate
        search={search}
        onSearchChange={setSearch}
        onSearch={() => (page === 1 ? refetch() : setPage(1))}
        onCreateClick={() => navigate({ to: '/datos/provider/create' })}
        createButtonText={t('providers.createProvider')}
